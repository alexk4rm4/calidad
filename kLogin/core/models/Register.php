...
final class Register extends Models implements OCREND {
  ...
  final public function SignUp(array $data) : array {
    ...
    return array('success' => 1, 'message' => 'Accediendo a método de autentificación.');
  }

  final public function SignUpAuth(array $data) : array {
    $error = $this->errors($data);
    if(!is_bool($error)) {
      return $error;
    }
    $e = array(
      'user' => $data['user'],
      'pass' => Strings::encriptar($data['pass']),
      'email' => $data['email'],
      'object' => $data['regObj'],
      'var1' => $data['regVar1'.$data['regObj']],
      'var2' => $data['regVar2'.$data['regObj']],
      'var3' => $data['regVar3'.$data['regObj']],
      'var4' => $data['regVar4'.$data['regObj']],
