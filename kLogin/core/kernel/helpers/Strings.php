...
final class Strings extends Twig_Extension {
  ...
  */
  ...
  final public static function encriptar($cadena){
    $key='adccvrjl';  // Una clave de codificacion, debe usarse la misma para encriptar y desencriptar
    $encrypted = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($key), $cadena, MCRYPT_MODE_CBC, md5(md5($key))));
    return $encrypted; //Devuelve el string encriptado
 
}
 
 final public static function desencriptar($cadena){
     $key='adccvrjl';  // Una clave de codificacion, debe usarse la misma para encriptar y desencriptar
